...
<head>
  ...
  transition: color 0.3s ease, text-shadow 0.3s ease; /* Thêm hiệu ứng chuyển đổi cho cả màu chữ và bóng */
}

.back-to-home a:hover {
  color: orange; /* Màu chữ khi hover (có thể thay đổi theo ý muốn) */
  text-shadow: 2px 2px 5px rgba(0, 0, 0, 0.5); /* Đổ bóng mờ khi hover (thêm bóng đen mờ xung quanh chữ) */
}
...
<div class="news-section">
  ...
  <div class="news-list">
    <%
      // Lấy danh sách tin tức từ request attribute
      List<TinTuc> newsList = new TinTucDAO().getAllTinTuc();
      if (newsList != null && !newsList.isEmpty()) {
        for (TinTuc tinTuc : newsList) {
    %>
    <div class="news-item">
      <div class="news-image">
        <img src="<%= tinTuc.getImage() %>" alt="<%= tinTuc.getTitle() %>" style="width: 100%; height: auto;">
      </div>
      <div class="news-content">
        <h3><%= tinTuc.getTitle() %></h3>
        <p><%= tinTuc.getContent() %></p>
        <p class="news-date">
          <%= new java.text.SimpleDateFormat("dd/MM/yyyy").format(tinTuc.getDatePosted()) %>
        </p>
        <a href="newsDetail?id=<%= tinTuc.getId() %>">Xem chi tiết</a>
